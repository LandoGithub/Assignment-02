...
public class LibraryGeneric<Type> {
	...
	public boolean checkout(long isbn, Type holder, int month, int day, int year) {
		...
		for (LibraryBookGeneric<Type> b : library) {
			if (b.getIsbn() == isbn && b.isCheckedIn() == true) {
	...
	public boolean checkin(long isbn) {
		for (LibraryBookGeneric<Type> b : library) {
			if (b.getIsbn() == isbn && b.isCheckedIn() == false) {
				b.checkIn();
				return true;
			}
		}
	...
	public boolean checkin(Type holder) {
		...
		holdersBooks = this.lookup(holder);
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		...
		ArrayList<LibraryBookGeneric<Type>> libraryBooksDue = new ArrayList<LibraryBookGeneric<Type>>();
		OrderByDueDate comparator = new OrderByDueDate();
		
		for (LibraryBookGeneric<Type> book : library) {
			if(book.isCheckedIn() == false) {
				if(comparator.compare(book, inputDate) < 0) {
					libraryBooksDue.add(book);
				}
			}
		}
		
		sort(libraryBooksDue, comparator);
		
		return libraryBooksDue;
	 ...
	 * 3. Reconsiders the list be the remaining unsorted portion (second item to Nth item) and repeats steps 1, 2, and 3.
	 */
	...
	protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> {
		...
		public int compare(LibraryBookGeneric<Type> lhs, GregorianCalendar rhs) {
			// The value is 0 if the times are equals
			// The Value is less than 0 if left hand is before right side
			// The value is greater than 0 if left hand is after right side
			return lhs.getDueDate().compareTo(rhs);
		}

		@Override
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			// The value is 0 if time equals
			// The Value is less than 0 if left hand is before right side
			// The value is greater than 0 if left hand is after right side
